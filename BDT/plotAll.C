...
namespace MY
{
    ...
    public:
        Double_t myKolmogorovTest(const TH1 *acca2, Option_t *option = "") const
        {
            ...
            if (opt.Contains("N") && !(afunc1 || afunc2))
            {
                ...
                if (prob > 0 && prb2 > 0)
                    prob *= prb2 * (1 - TMath::Log(prob * prb2));
                else
                    prob = 0;
            }
        ...
        for (Int_t i = 0; i < nPlots; i++)
        {
            ...
            if (h)
            {
                ...
                if (hname[i] == "NodesBeforePruning")
                    h->SetTitle("Nodes before/after pruning");
                h->Draw();
                if (hname[i] == "NodesBeforePruning")
                {
                ...
                if (hname[i] == "BoostMonitor")
                { // a plot only available in case DoBoostMontior option has bee set
                    TGraph *g = (TGraph *)bdtdir->Get("BoostMonitorGraph");
                    g->Draw("LP*");
                    BoostMonitorIsDone = kTRUE;
                }
                if (hname[i] == titName + "_FOMvsIterFrame")
                { // a plot only available in case DoBoostMontior option has bee set
              ...
              Bool_t useTMVAStyle = kFALSE)
    {
        ...
        while ((key = (TKey *)next()))
        {
            ...
            while ((titkey = (TKey *)keyIt()))
            {
                ...
                // create new canvas
                TString ctitle = ((htype == kMVAType) ? Form("TMVA response %s", methodTitle.Data()) : (htype == kProbaType) ? Form("TMVA probability %s", methodTitle.Data())
                                                                                                   : (htype == kCompareType) ? Form("TMVA comparison %s", methodTitle.Data())
                ...
                if (htype == kProbaType)
                    frame->GetXaxis()->SetTitle("Signal probability");
                else if (htype == kRarityType)
                    frame->GetXaxis()->SetTitle("Signal rarity");
                           ...
                           TString outdir)
    {
        ...
        while ((xkey = (TKey *)xnext()))
        {
            ...
            if (ninst == 0)
                cout << "hmm... sorry, but this printout was supposed to be only to keep the compiler quite.. never supposed to happen :(" << endl;
                   ...
                   Bool_t useTMVAStyle = true)
    {
        ...
        while ((key = (TKey *)next()))
        {
            ...
            if (!TString(key->GetName()).Contains("__Signal") &&
                !(isRegression && TString(key->GetName()).Contains("__Regression")))
                continue;
